...
__doc__ = """
	Extract all pairs of genes (adjacencies) conserved between each pair of In-In species.
"""
...
for anc in details:
    ...
    # -1 is the outgroup, 1,2,3... are the descendants
    ...
    for ancPair in details[anc]:
        ...
        print >> sys.stderr, "weigths:", weights
        for (x, y) in itertools.combinations(weights.keys(), 2):
            print >> sys.stderr, "x,y:", x, y
            if (x > 0) and (y > 0):
                print >> sys.stderr, "InIn:", weights[x], weights[y]
                weightInIn += (weights[x] * weights[y])
